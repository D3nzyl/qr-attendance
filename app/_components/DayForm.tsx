...
function getEmpCols(employees: Employee[]): string[] {
  const seen = new Set<string>();
  employees.forEach((e) =>
    Object.keys(e).forEach((k) => {
      if (!INTERNAL.has(k)) seen.add(k);
    }),
  );
  return [...seen];
}
...
function getCanvasPos(e: CanvasPointerEvent, canvas: HTMLCanvasElement) {
...
interface SafetyTopicPickerProps {
  selected: number[];
  onToggle: (id: number, checked: boolean) => void;
  invalid?: boolean;
}

function SafetyTopicPicker({ selected, onToggle }: SafetyTopicPickerProps) {
  ...
  const filtered = SAFETY_TOPICS.filter(
    (t) =>
            ...
            filtered.map((t) => {
              ...
              return (
                ...
                >
                  ...
                  {isSel && (
                    <span className="safety-combo-check">&#10003;</span>
                  )}
                </div>
              );
            })
...
interface EmpSignatureCellProps {
  emp: Employee;
  onChange: (dataUrl: string | null) => void;
  invalid?: boolean;
}

function EmpSignatureCell({ emp, onChange, invalid }: EmpSignatureCellProps) {
  ...
  const start = (e: CanvasPointerEvent) => {
    e.preventDefault();
  ...
  const move = (e: CanvasPointerEvent) => {
...
export default function DayForm({
  ...
  const [form, setForm] = useState<DayData>(() => {
    const base = { ...blank(), ...(initial || {}) } as DayData;
    const rawLocs = base.locations as unknown;
    if (typeof rawLocs === "string") {
      base.locations = rawLocs ? [rawLocs] : [];
    }
    return base;
  });
  const [scanning, setScanning] = useState(false);
  const [showManual, setShowManual] = useState(false);
  const [manualEmp, setManualEmp] = useState<ManualEmp>({
    nric: "",
    id: "",
    name: "",
  });
  ...
  const [viewImage, setViewImage] = useState<ImageRef | null>(null);
  const formRef = useRef<HTMLDivElement>(null);

  const getGpsLocation = (targetIdx: number) => {
  ...
  const set = <K extends keyof DayData>(key: K, val: DayData[K]) => {
    setForm((prev) => ({ ...prev, [key]: val }));
    setDirty(true);
  };

  handlerRef.current = (text: string) => {
    const now = Date.now();
    if (
    ...
    try {
      ...
      setForm((prev) => ({
        ...
        employees: [
          ...
          {
            id: Date.now(),
            scannedAt: new Date().toLocaleTimeString("en-SG", {
              hour12: false,
            }),
            _original: text,
            ...data,
  ...
  const onScan = (text: string) => handlerRef.current?.(text);

  const removeEmployee = (id: number) =>
    set(
      "employees",
      form.employees.filter((e) => e.id !== id),
    );
  ...
  const updateEmpTemp = (id: number, tempStr: string) => {
  ...
  const setEmpSignature = (id: number, dataUrl: string | null) => {
    setForm((prev) => ({
      ...prev,
      employees: prev.employees.map((e) =>
        e.id === id ? { ...e, signature: dataUrl } : e,
      ),
    }));
    setDirty(true);
  };

  const toggleSafetyTopic = (id: number, checked: boolean) => {
    setForm((prev) => {
      const ids = checked
  ...
  const setSafetyDesc = (id: number, text: string) => {
    setForm((prev) => ({
      ...prev,
  ...
  const getEmpLabel = (emp: Employee): string | number | boolean => {
    const nameKey = Object.keys(emp).find(
      (k) => /name/i.test(k) && !INTERNAL.has(k),
    );
    return (
      (nameKey && emp[nameKey]) ||
      Object.entries(emp).find(([k]) => /nric|wp/i.test(k))?.[1] ||
      "Employee"
    );
  };

  const validate = (): { errors: string[]; fields: Set<string> } => {
    const errors: string[] = [];
    ...
    if (form.safetyTopicIds.length < 2) {
      ...
      const noDesc = form.safetyTopicIds.filter(
        (id) => !form.safetyTopicDescs[id]?.trim(),
      );
      if (noDesc.length > 0) {
    ...
    if (form.employees.length === 0) {
      ...
      form.employees.forEach((emp) => {
        ...
        if (!hastemp) {
          errors.push(`${label}: temperature not entered.`);
          fields.add(`emp-temp-${emp.id}`);
        }
        if (hastemp && !tooHot && !emp.fit) {
          errors.push(`${label}: not marked Fit for Work.`);
          fields.add(`emp-fit-${emp.id}`);
        }
        if (!emp.signature) {
          errors.push(`${label}: signature missing.`);
          fields.add(`emp-sig-${emp.id}`);
        }
      });
    }
    ...
    if (!form.conductedBy?.trim()) {
      ...
      fields.add("conductedBy");
    }
    if (!form.signature) {
      errors.push("Supervisor signature is required.");
      fields.add("supervisorSig");
    }
      ...
      <div className="day-form-body" ref={formRef}>
        ...
        <div className="form-group">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {(form.locations.length > 0 ? form.locations : [""]).map(
              (loc, idx) => (
                <div
                  key={idx}
                  ...
                  {form.locations.length > 1 && (
                    <button
                      ...
                      onClick={() =>
                        set(
                          "locations",
                          form.locations.filter((_, i) => i !== idx),
                        )
                      }
                    >
                      ✕
            ...
            <button
              ...
              onClick={() =>
                set("locations", [
                  ...(form.locations.length ? form.locations : [""]),
                  "",
                ])
              }
            >
        ...
        >
          <label
            style={{
              color: invalidFields.has("employees")
                ? "var(--danger)"
                : undefined,
            }}
          >
            Employee — Safety &amp; Health Declaration
            {invalidFields.has("employees") && (
              <span style={{ fontSize: 12, marginLeft: 6 }}>⚠ Required</span>
            )}
          </label>
              ...
              <div className="manual-add-form">
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setManualEmp((p) => ({ ...p, nric: e.target.value }))
                    }
                  />
                </div>
                <div className="form-group">
                  ...
                  <input
                    ...
                    value={manualEmp.id}
                    onChange={(e) =>
                      setManualEmp((p) => ({ ...p, id: e.target.value }))
                    }
                  />
                </div>
                <div className="form-group">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setManualEmp((p) => ({ ...p, name: e.target.value }))
                    }
                ...
                {form.employees.map((emp, i) => {
                  ...
                  return (
                    <div
                      ...
                      data-invalid={
                        invalidFields.has(`emp-temp-${emp.id}`) ||
                        invalidFields.has(`emp-fit-${emp.id}`) ||
                        invalidFields.has(`emp-sig-${emp.id}`)
                          ? "true"
                          : undefined
                      }
                    >
                      <div className="emp-card-header">
                        ...
                        <div className="emp-card-info">
                          ...
                          {["nric", "NRIC/WP No.", "ID", "name", "Name"]
                            .filter((c) => emp[c] != null)
                            .map((c) => (
                              <span key={c} className="emp-card-chip">
                                <span className="emp-chip-label">{c}:</span>{" "}
                                {emp[c] ?? "—"}
                              </span>
                            ))}
                          {empCols
                      ...
                      <div className="emp-card-body">
                        <div className="emp-health-row">
                          ...
                          >
                            <label
                              style={{
                                color: invalidFields.has(`emp-temp-${emp.id}`)
                                  ? "var(--danger)"
                                  : undefined,
                              }}
                            >
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                updateEmpTemp(emp.id, e.target.value)
                              }
                            />
                          </div>
                          <div
                        ...
                        >
                          <label
                            style={{
                              color: invalidFields.has(`emp-sig-${emp.id}`)
                                ? "var(--danger)"
                                : undefined,
                            }}
                          >
                ...
                <table className="emp-desktop-table">
                  ...
                  <tbody>
                    {form.employees.map((emp, i) => {
                      ...
                      const nric =
                        emp.nric || emp["NRIC/WP No."] || emp.NRIC || "—";
                      const empId = emp.empId || emp.ID || emp.id || "—";
                      return (
                        <tr
                          key={emp.id}
                          className={rowInvalid ? "emp-row-invalid" : ""}
        ...
        >
          <label
            style={{
              color: invalidFields.has("topics") ? "var(--danger)" : undefined,
            }}
          >
          ...
          {form.safetyTopicIds.length > 0 && (
            <div className="safety-descs">
              ...
              >
                Write a brief for each selected topic (min. 2 topics, approx.
                ...
                .map((id) => {
                  ...
                  return (
                    ...
                    >
                      <label
                        style={{
                          color: descInvalid ? "var(--danger)" : undefined,
                        }}
                      >
          ...
          {form.images.length < 5 && (
            <label
              ...
              style={{
                ...
                pointerEvents: imgUploading ? "none" : undefined,
              }}
            >
        ...
        >
          <label
            style={{
              color: invalidFields.has("conductedBy")
                ? "var(--danger)"
                : undefined,
            }}
          >
        ...
        >
          <label
            style={{
              color: invalidFields.has("supervisorSig")
                ? "var(--danger)"
                : undefined,
            }}
          >
...
function SignaturePad({ value, onChange, invalid }: SignaturePadProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const drawing = useRef(false);
  const restored = useRef(false);
