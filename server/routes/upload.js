...
router.post('/:contract/:teamNo/:weekKey/:day', upload.single('image'), (req, res) => {
...
router.delete('/:imageId', (req, res) => {
  const row = db.prepare('SELECT filename FROM images WHERE id=?').get(req.params.imageId);
  ...
  import('fs').then(({ unlinkSync }) => {
    try { unlinkSync(`uploads/${row.filename}`); } catch {}
  });
