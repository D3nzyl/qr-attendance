...
export default function DayForm({ dayKey, date, initial, onSave, onClear, contract, teamNo, weekKey }: DayFormProps) {
  ...
  const [manualEmp, setManualEmp] = useState<ManualEmp>({ nric: '', id: '', name: '' });
  ...
  const removeEmployee = (id: number) => set('employees', form.employees.filter(e => e.id !== id));
  ...
  const updateEmpField = (id: number, field: string, value: string | boolean) => {
    setForm(prev => ({
      ...prev,
